/datum/wound/slash/bisection
	name = "bisection"
	check_name = "<span class='userdanger'><B>BISECTED</B></span>"
	severity = WOUND_SEVERITY_FATAL
	crit_message = list(
		"%VICTIM is messily bisected!",
		"%VICTIM is split into two!",
		"%VICTIM has fallen apart!",
	)
	...
	woundpain = 100
	sewn_woundpain = 20
	sew_threshold = 200 //who knew its hard to fix being cut in half
	critical = TRUE
	...
	/// Organs still spill on bisection.
	var/static/list/affected_organs = list(
		ORGAN_SLOT_STOMACH = 100,
		ORGAN_SLOT_LIVER = 50,
	)

/datum/wound/slash/bisection/can_stack_with(datum/wound/other)
	if(istype(other, /datum/wound/slash/bisection) && (type == other.type))
		return FALSE
	return TRUE

/datum/wound/slash/bisection/can_apply_to_bodypart(obj/item/bodypart/new_limb)
...
/datum/wound/slash/bisection/on_mob_gain(mob/living/affected)
	. = ..()
	affected.emote("paincrit", TRUE)
	affected.Slowdown(20)
	shake_camera(affected, 2, 2)

/datum/wound/slash/bisection/on_bodypart_gain(obj/item/bodypart/affected)
	. = ..()
